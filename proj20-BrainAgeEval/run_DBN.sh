...
for SCAN in `ls $PROJECT_DIR/mass_skullstrip/*_brain.nii.gz` ; do
	...
	if [[ ! -f ${PROJECT_DIR}/analysis/brainages_DBN/${SUB}_${SES}/Subject${SUB}${SES}_T1_BrainAligned.nii.gz ]] ; then	
		echo "Extracting the Brain & Standardizing Intensity For SUB: ${SUB} SES: ${SES} "
		mkdir -p $PROJECT_DIR/analysis/brainages_DBN/${SUB}_${SES}
		OUTPUT=`echo $PROJECT_DIR/analysis/brainages_DBN/${SUB}_${SES}/Subject${SUB}${SES}_T1_BrainAligned.nii.gz`
		flirt -in $SCAN -ref $SCRIPTS_DIR/mass_1.1.1/data/tpl-MNI152_T1_1mm_brain_LPS_filled.nii.gz -dof 12 -out $OUTPUT
	fi
done
COUNT=`ls ${PROJECT_DIR}/analysis/brainages_DBN/*_*/*.nii.gz | wc -l`
fslmerge -t ${PROJECT_DIR}/analysis/brainages_DBN/n${COUNT}_skullstripped.nii.gz `ls ${PROJECT_DIR}/analysis/brainages_DBN/*_*/*.nii.gz`

#########
### Split the Input Scans Into 80 Sreenshots
#########

for INPUT in `echo ${PROJECT_DIR}/analysis/brainages_DBN/*/*.nii.gz` ; do
...
#	${PROJECT_DIR}/analysis/brainages_DBN \
#	${PROJECT_DIR}/analysis/brainages_DBN/output.csv \
#	${SCRIPTS_DIR}/DeepBrainNet/CBICA/DBN_model.h5

#LOCAL_DIR=/Users/Jirsaraie/Desktop/Research/proj20-BrainAgeEval
#rsync -auv rjirsara@login.chpc.wustl.edu:/scratch/rjirsara/proj20-BrainAgeEval/analysis/brainages_DBN ${LOCAL_DIR}
#python3 ${LOCAL_DIR}/DeepBrainNet/Script/Model_Test.py ${LOCAL_DIR}/analysis/brainages_DBN ${LOCAL_DIR}/analysis/brainages_DBN/output.csv ${LOCAL_DIR}/DeepBrainNet/CBICA/DBN_model.h5
#rsync -auv ${LOCAL_DIR}/analysis/brainages_DBN/output.csv rjirsara@login.chpc.wustl.edu:/scratch/rjirsara/proj20-BrainAgeEval/analysis/brainages_DBN
